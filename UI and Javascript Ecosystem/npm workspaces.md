...
## Enter workspaces[​](#definition "Direct link to Enter workspaces")
...
## A brief history[​](#a-brief-history "Direct link to A brief history")
...
![](https://wasp.sh/assets/images/timeline-1df44e79015fc28405342ed769263f3b.png)
...
## Problems and solutions[​](#problems-and-solutions "Direct link to Problems and solutions")
...
### The dependencies of the whole project are resolved together[​](#the-dependencies-of-the-whole-project-are-resolved-together "Direct link to The dependencies of the whole project are resolved together")
...
Multiple copies aren't just a waste of disk space, they can cause bugs. In JavaScript, the same class defined in two different files is considered _different_, even if the code is identical[2](#user-content-fn-react-aside-f1ae0a):

main.js

```
import * as foo from "./foo/my-class";import * as bar from "./bar/my-class";new foo.MyClass() instanceof foo.MyClass; // truenew foo.MyClass() instanceof bar.MyClass; // false!
```
...
### Workspaces can depend on each other by name[​](#workspaces-can-depend-on-each-other-by-name "Direct link to Workspaces can depend on each other by name")

#### The problem[​](#the-problem-1 "Direct link to The problem")
...
#### The solution[​](#the-solution-1 "Direct link to The solution")
...
If you've ever looked into your `node_modules` and found dependencies nested inside other dependencies instead of a flat structure, this is why. The package manager has no influence on how Node.js finds imports[3](#user-content-fn-integrated-package-manager-f1ae0a), all it can do is lay them out on disk in a way that Node.js will understand.

##### How Node.js finds packages[​](#how-nodejs-finds-packages "Direct link to How Node.js finds packages")
...
##### Symlinks tie it all together[​](#symlinks-tie-it-all-together "Direct link to Symlinks tie it all together")
...
## When (not) to use workspaces[​](#when-not-to-use-workspaces "Direct link to When (not) to use workspaces")
...
-   You have cyclic dependencies. Without a clear story of which packages depend on which[4](#user-content-fn-recommend-yarn-f1ae0a), workspaces can allow any package depend on any other carelessly, and that can lead to a tangled web of dependencies.
...
## Why does Wasp care about workspaces?[​](#why-does-wasp-care-about-workspaces "Direct link to Why does Wasp care about workspaces?")
...
1.  Yarn has a great [glossary](https://yarnpkg.com/advanced/lexicon) that you can check out if you want to get specific definitions for a package-manager-related term, and pointers to which feature they're a part of. [↩](#user-content-fnref-yarn-glossary-f1ae0a)
    
2.  This is one of the reasons React requires a single copy in your app. React relies on a single instance to track internal state (hooks, context, and reconciler). If you had two copies, components created with one wouldn't be recognized by the other, leading to cryptic errors. [↩](#user-content-fnref-react-aside-f1ae0a)
    
3.  Some newer runtimes have the package manager integrated directly into execution, so they can control both installation and resolution, and simply point to the right folder. For example, Deno doesn't even install dependencies in your project folder. Instead, it keeps a single global copy in an internal cache that it can programmatically reference. [↩](#user-content-fnref-integrated-package-manager-f1ae0a)
    
4.  Yarn has some great features that force you to be exhaustive about how your workspaces are structured. By default, you have to [explicitly declare which other workspaces you depend on](https://yarnpkg.com/features/workspaces#cross-references). They also have a [workspace constraints feature](https://yarnpkg.com/features/constraints) that lets you enforce rules on which packages can depend on which others. This one has a steep learning curve, but it's worth it if you're working on a very big monorepo. [↩](#user-content-fnref-recommend-yarn-f1ae0a)