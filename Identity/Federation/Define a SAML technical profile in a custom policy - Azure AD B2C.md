...
![metadata and certificate exchange](https://learn.microsoft.com/en-us/azure/active-directory-b2c/media/saml-technical-profile/technical-profile-idp-saml-metadata.png)
...
-   The **first\_name** claim is mapped to the **givenName** claim.
-   The **last\_name** claim is mapped to the **surname** claim.
...
| IncludeClaimResolvingInClaimsHandling | No | For input and output claims, specifies whether [claims resolution](claim-resolver-overview) is included in the technical profile. Possible values: `true`, or `false` (default). If you want to use a claims resolver in the technical profile, set this to `true`. |
| SingleLogoutEnabled | No | Indicates whether during sign-in the technical profile attempts to sign out from federated identity providers. For more information, see [Azure AD B2C session sign-out](session-behavior#sign-out). Possible values: `true` (default), or `false`. |
...
-   [Add ADFS as a SAML identity provider using custom policies](identity-provider-adfs)
-   [Sign in by using Salesforce accounts via SAML](identity-provider-salesforce-saml)